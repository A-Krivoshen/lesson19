...
";
        ...
        return json_encode($this->sendQueryToDb($pdo, $query, [$id])->fetch(\PDO::FETCH_ASSOC));
    }
    ...
    public function addTask()
    {
        ...
        $this->sendQueryToDb($pdo, $query, [$description, $id, $id]);
    }
    ...
    public function editTask()
    {
        ...
        if (!trim($description) || strlen(trim($description)) === 0) {
            die('задача из пробелов? ...'); // Jquery не видит этот die и выдает success, исправить!
        }
        $id = (int)!empty($_POST['id']) ? $_POST['id'] : 0;
